...
public class GetRemittanceAPITest  {
    ...
    /*
        #Usecase - 1.A - **RETRIEVE PAYMENT BY MASTERCARD PROVIDED ID**
    ...
    */
    ...
    public void testGetPaymentByIdAfterSuccessfulPayment() {
        logger.info("Running Usecase - 1.A, RETRIEVE PAYMENT BY MASTERCARD PROVIDED ID.");
        try {
    ...
    /*
         #Usecase - 1.B - **RETRIEVE PAYMENT IN ENCRYPTED FORM**
    ...
    public void testGetPaymentByIdWithEncryptionAfterSuccessfulPayment() {
        if (apiConfig.getRunWithEncryptedPayload()) {
            logger.info("Running Usecase - 1.B, RETRIEVE PAYMENT IN ENCRYPTED FORM.");
            try {
                Map<String, Object> paymentParams = new HashMap<>();
                paymentParams.put(partnerIdStr, partnerId);
                HttpHeaders headers = new HttpHeaders();
    ...
    /*
         #Usecase - 2.A - **RETRIEVE PAYMENT BY OI PROVIDED TRANSACTION REFERENCE ID**
    ...
    public void testGetPaymentByReferenceAfterSuccessfulPayment() {
        logger.info("Running Usecase - 2.A, RETRIEVE PAYMENT BY OI PROVIDED TRANSACTION REFERENCE ID.");
        try {
            Map<String, Object> paymentParams = new HashMap<>();
            paymentParams.put(partnerIdStr, "partnerId");
            HttpHeaders headers = new HttpHeaders();
    ...
    /*
         #Usecase - 2.B - **RETRIEVE PAYMENT IN ENCRYPTED FORM**
         It will first make a payment and then get its details of this transaction using paymentId.
         All these calls are encrypted.
       */
    @Test
    public void testGetPaymentByReferenceWithEncryptionAfterSuccessfulPayment() {
        if (apiConfig.getRunWithEncryptedPayload()) {
            logger.info("Running Usecase - 2.B, RETRIEVE PAYMENT IN ENCRYPTED FORM.");
            try {
                Map<String, Object> paymentParams = new HashMap<>();
                paymentParams.put(partnerIdStr, "partnerId");
                HttpHeaders headers = new HttpHeaders();
                ...
                if (null != paymentDetails) {
                    ...
                    RemittanceResponse retrievePayment = getRemittanceAPI.getPaymentByRefWithEncryption(headers, requestParams);
                    if (null != retrievePayment) {
                        logger.info("Retrieve payment by reference is Successful");
                        Assert.assertEquals(retrievePayment.getTransactionReference(), transactionReference);
                    } else {
    ...
    /*
      #Usecase - 3 - **RETRIEVE PAYMENT BY OI PROVIDED TRANSACTION REFERENCE ID IN JSON FORMAT**
     It will first make a payment and then get its details of this transaction using its transaction_reference
     Both these API calls will be done using JSON format.
   */
    @Test
    public void testGetPaymentByReferenceInJsonFormat() {
        logger.info("Running Usecase - 3, RETRIEVE PAYMENT BY OI PROVIDED TRANSACTION REFERENCE ID IN JSON FORMAT.");
        try {
            Map<String, Object> paymentParams = new HashMap<>();
            paymentParams.put(partnerIdStr, "partnerId");
            HttpHeaders headers = new HttpHeaders();
