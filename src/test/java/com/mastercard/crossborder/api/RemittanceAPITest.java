...
public class RemittanceAPITest  {
    ...
    /*
       #Usecase - 1.A - **PAYMENT WITH FORWARD QUOTE**
    ...
    */
    ...
    public void testPaymentWithQuote() {
        logger.info("Running Usecase - 1.A, PAYMENT WITH FORWARD QUOTE.");
    ...
    /*
       #Usecase - 1.B - **PAYMENT WITH FORWARD QUOTE WITH ENCRYPTION**
       Make a forward quote and use same proposal ID to make payment
       NOTE: Proposal returned from quotes response is time bound
    */
    @Test
    public void testPaymentWithQuoteWithEncryption() {
        if (apiConfig.getRunWithEncryptedPayload()) {
            logger.info("Running Usecase - 1.B, PAYMENT WITH FORWARD QUOTE WITH ENCRYPTION.");
            Map<String, Object> requestParams = new HashMap<>();
            requestParams.put("partner-id", partnerId);
            HttpHeaders headers = new HttpHeaders();
            headers.add(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_XML);
    ...
    /*
      #Usecase - 2.A - **FORWARD PAYMENT WITHOUT QUOTE WITH NOT FEES INCLUDED FOR PERSON TO BUSINESS PAYMENT TYPE**
   ...
   */
    ...
    public void testOneShotPaymentWithForwardQuote() {
        logger.info("Running Usecase - 2.A, FORWARD PAYMENT WITHOUT QUOTE WITH NOT FEES INCLUDED FOR PERSON TO BUSINESS PAYMENT TYPE");
    ...
    /* #Usecase - 2.B - **PAYMENT WITH ENCRYPTION SUPPORTED**
   ...
   */
    ...
    public void testOneShotPaymentWithEncryption() {
        if (apiConfig.getRunWithEncryptedPayload()) {
            logger.info("Running Usecase - 2.B, PAYMENT WITH ENCRYPTION SUPPORTED.");
            Map<String, Object> requestParams = new HashMap<>();
            requestParams.put("partner-id", partnerId);
            HttpHeaders headers = new HttpHeaders();
            headers.add(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_XML);
   ...
   */
    ...
    /*
         #Usecase - 4.A - **REVERSE PAYMENT WITHOUT QUOTE FOR BUSINESS TO PERSON PAYMENT TYPE**
    ...
    public void testOneShotPaymentWithReverseQuote() {
        logger.info("Running Usecase - 4.A,REVERSE PAYMENT WITHOUT QUOTE FOR BUSINESS TO PERSON PAYMENT TYPE");
    ...
    /*
         #Usecase - 4.B - **REVERSE PAYMENT WITHOUT QUOTE FOR BUSINESS TO PERSON PAYMENT TYPE WITH ENCRYPTION**
         There is no separate call made to get the quote, in a single API call we are making payment
      */
    @Test
    public void testOneShotPaymentWithReverseQuoteWithEncryption() {
        if (apiConfig.getRunWithEncryptedPayload()) {
            logger.info("Running Usecase - 4.B,REVERSE PAYMENT WITHOUT QUOTE FOR BUSINESS TO PERSON PAYMENT TYPE WITH ENCRYPTION");
            Map<String, Object> requestParams = new HashMap<>();
            requestParams.put("partner-id", partnerId);
            HttpHeaders headers = new HttpHeaders();
            headers.add(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_XML);
    ...
    /*
      #Usecase - 5.A - **FORWARD PAYMENT WITHOUT QUOTE FOR BUSINESS TO BUSINESS PAYMENT TYPE**
      There is no separate call made to get the quote, in single API call we are making payment.
   */
    @Test
    ...
    /*
      #Usecase - 5.B - **FORWARD PAYMENT WITHOUT QUOTE FOR BUSINESS TO BUSINESS PAYMENT TYPE WITH ENCRYPTION**
      There is no separate call made to get the quote, in single API call we are making payment.
   */
    @Test
    public void testOneShotPaymentForBusinessToBusinessWithEncryption() {
        if (apiConfig.getRunWithEncryptedPayload()) {
            logger.info("Running Usecase - 5, FORWARD PAYMENT WITHOUT QUOTE FOR BUSINESS TO BUSINESS PAYMENT TYPE WITH ENCRYPTION");
    ...
    /*
      #Usecase - 6.A - **FORWARD PAYMENT WITHOUT QUOTE FOR GOVERNMENT TO PERSON PAYMENT TYPE**
      There is no separate call made to get the quote, in single API call we are making payment.
   */
    @Test
    ...
    /*
      #Usecase - 6.B - **FORWARD PAYMENT WITHOUT QUOTE FOR GOVERNMENT TO PERSON PAYMENT TYPE WITH ENCRYPTION**
      There is no separate call made to get the quote, in single API call we are making payment.
   */
    @Test
    public void testOneShotPaymentForGovernmentToPersonWithEncryption() {
        if (apiConfig.getRunWithEncryptedPayload()) {
            logger.info("Running Usecase - 6, FORWARD PAYMENT WITHOUT QUOTE FOR GOVERNMENT TO PERSON PAYMENT TYPE WITH ENCRYPTION");
   ...
   */
    ...
    /*
       #Usecase - 8 - **PAYMENT WITH FORWARD QUOTE IN JSON FORMAT**
       Make a forward quote and use same proposal ID to make payment
    ...
    */
    ...
    public void testPaymentWithQuoteInJsonFormat() {
        logger.info("Running Usecase - 8, PAYMENT WITH FORWARD QUOTE in Json format.");
        Map<String, Object> requestParams = new HashMap<>();
        requestParams.put("partner-id", partnerId);
        HttpHeaders headers = new HttpHeaders();
    ...
    /*
      #Usecase - 9 - **ERROR HANDLING IN JSON FORMAT**
      Payment can fail for various reasons, this scenario is added so that user should know what to expect where there is a failure
      Refer https://developer.mastercard.com/documentation/mastercard-send-cross-border/1#error-codes for details
   */
    @Test
    public void testErrorHandlingInJsonFormat() {
        logger.info("Running Usecase - 9, ERROR HANDLING IN JSON FORMAT.");
        Map<String, Object> requestParams = new HashMap<>();
        requestParams.put("partner-id", partnerId);
        HttpHeaders headers = new HttpHeaders();
        headers.add(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON);
